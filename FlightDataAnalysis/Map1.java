        ...
        throws IOException
    {
        ...
        for(int length = line.length(); i < length && f == 0; i++)
        {
            ...
            if(c == ',')
                count++;
            char k;
            for(; i < length && count == 8; key1 = (new StringBuilder(String.valueOf(key1))).append(k).toString())
            {
                k = line.charAt(++i);
                if(k != ',')
                    continue;
            ...
            char v;
            for(; i < length && count == 14; value1 = (new StringBuilder(String.valueOf(value1))).append(v).toString())
            {
                v = line.charAt(++i);
                if(v != ',')
                    continue;
                ...
                if(!value1.matches("[A-Za-z]+"))
                    output.collect(new Text(key1), new IntWritable(Integer.parseInt(value1) > 0 ? 0 : 1));
                f = 1;
                break;
            }
